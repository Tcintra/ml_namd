      SUBROUTINE ADIAB_NVE_I(p,p_e,p_s,d_c,w,Hmat,vect,HF,d,qs,c,Wb,Qb,Vs,Vb) 
...
! Define first nlv eigenvectors.
      do j=1,nlv
        ...
        enddo
      enddo
! Check dot product of eigenvectors with first eigenvector
!       do j=1,nlv
!         vdot(j)=0.d0
!       enddo
!       do j=2,nlv
!          do i=1,n
!             vdot(j)=vdot(j)+vect(i,j)*vect(i,1)
!          enddo
!          if(vdot(j).lt.0) then
!              do i=1,n
!                vect(i,j)=-vect(i,j)
...
!          endif
!       enddo      
! Make first element of each eigenvector positive
       do j=1,nlv
          if(vect(1,j).lt.0.d0) then
              do i=1,n
                vect(i,j)=-vect(i,j)
              enddo
          endif
