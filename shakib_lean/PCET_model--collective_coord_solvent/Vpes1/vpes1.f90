...
! This program integrates the quantum Hamiltonian of the 1st submatrix.
      ...
      real*8 qp,qe,qp1,qp2,qe1,qe2,qp0,fact
      real*8 pi
      PARAMETER (nmax=30) 
      PARAMETER (pi=3.14159d0)
      DOUBLE PRECISION :: phip1(0:nmax),phip2(0:nmax),phie1(0:nmax),phie2(0:nmax)
      DOUBLE PRECISION :: pl(0:nmax),Vpes1_ac(0:nmax,0:nmax,0:nmax,0:nmax), &
      Vpes1_bc(0:nmax,0:nmax,0:nmax,0:nmax),Vpes1_ad(0:nmax,0:nmax,0:nmax,0:nmax), &
      Vpes1_bd(0:nmax,0:nmax,0:nmax,0:nmax),Vpes1_inta(0:nmax,0:nmax,0:nmax,0:nmax), &
      Vpes1_intb(0:nmax,0:nmax,0:nmax,0:nmax),Vpes1(0:nmax,0:nmax,0:nmax,0:nmax),dpl(0:nmax)
...
!--------------------------------------------------------------------
! Calculate the protonic part of solvent coupled potential (Vpes1) 
       ...
       do j=0,ne
        do p=0,np
         do r=0,ne
          ...
          V1=-(qp-qp0)
          Vpes1_ac(i,j,p,r)=phip1(i)*phie1(j)*V1*phip2(p)*phie2(r)
          ...
          phie2(r)=amp2*(2**r*fact(r)*dsqrt(pi))**(-0.5)*b2**0.5*pl(r)*dexp(-(b2**2*(qe-qe2)**2)/2)
          V1=-(qp-qp0)
          Vpes1_bc(i,j,p,r)=phip1(i)*phie1(j)*V1*phip2(p)*phie2(r)
          
          

          sum=0.5d0*(Vpes1_ac(i,j,p,r)+Vpes1_bc(i,j,p,r))
          do lp=1,mp-1
            qp=a+lp*hp
            xp1=b1*(qp-qp1)
            xp2=b1*(qp-qp2)
          ...
          phie2(r)=amp2*(2**r*fact(r)*dsqrt(pi))**(-0.5)*b2**0.5*pl(r)*dexp(-(b2**2*(qe-qe2)**2)/2)
          V1=-(qp-qp0)
          sum=sum+phip1(i)*phie1(j)*V1*phip2(p)*phie2(r)
          enddo
          ...
          phie2(r)=amp2*(2**r*fact(r)*dsqrt(pi))**(-0.5)*b2**0.5*pl(r)*dexp(-(b2**2*(qe-qe2)**2)/2)
          V1=-(qp-qp0)
          Vpes1_ad(i,j,p,r)=phip1(i)*phie1(j)*V1*phip2(p)*phie2(r)
          ...
          phie2(r)=amp2*(2**r*fact(r)*dsqrt(pi))**(-0.5)*b2**0.5*pl(r)*dexp(-(b2**2*(qe-qe2)**2)/2)
          V1=-(qp-qp0)
          Vpes1_bd(i,j,p,r)=phip1(i)*phie1(j)*V1*phip2(p)*phie2(r)
          
          sum=sum+0.5d0*(Vpes1_ad(i,j,p,r)+Vpes1_bd(i,j,p,r))
          do lp=1,mp-1
            qp=a+lp*hp
            xp1=b1*(qp-qp1)
            xp2=b1*(qp-qp2)
          ...
          phie2(r)=amp2*(2**r*fact(r)*dsqrt(pi))**(-0.5)*b2**0.5*pl(r)*dexp(-(b2**2*(qe-qe2)**2)/2)
          V1=-(qp-qp0)
          Vpes1_inta(i,j,p,r)=phip1(i)*phie1(j)*V1*phip2(p)*phie2(r)
          ...
          phie2(r)=amp2*(2**r*fact(r)*dsqrt(pi))**(-0.5)*b2**0.5*pl(r)*dexp(-(b2**2*(qe-qe2)**2)/2)
          V1=-(qp-qp0)
          Vpes1_intb(i,j,p,r)=phip1(i)*phie1(j)*V1*phip2(p)*phie2(r)
         
          sum=sum+0.5d0*(Vpes1_inta(i,j,p,r)+Vpes1_intb(i,j,p,r))
          do lp=1,mp-1
            qp=a+lp*hp
            if (qp==qe) cycle
            xp1=b1*(qp-qp1)
          ...
          enddo
          enddo
          Vpes1(i,j,p,r)=hp*he*sum
          write(100,*) i,j,p,r,Vpes1(i,j,p,r)
         enddo
        enddo
       enddo
      enddo
